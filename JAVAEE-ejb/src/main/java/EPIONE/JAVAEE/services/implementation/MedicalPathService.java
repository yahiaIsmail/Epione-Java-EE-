...
public class MedicalPathService implements MedicalPathServiceLocal {
	...
	public List<MedicalPath> pathsforOneDoctor(int id) {
		TypedQuery<MedicalPath> list = em.createQuery(
				"select p from MedicalPath p " + "where p.rendezVous.doctors.id=:id   ", MedicalPath.class);

		List<MedicalPath> query;
		query = list.setParameter("id", id).getResultList();
		for (MedicalPath p : query) {
			List<PathDoctors> personsWithoutDuplicates = p.getDoctorPath().stream().distinct()
	...
	public List<RDV> getDocRDVS(int id) {
		TypedQuery<RDV> list = em.createQuery(
				"select p from RDV p WHERE NOT EXISTS (SELECT m FROM MedicalPath m where p.id= m.rendezVous.id)and p.doctors.id=:id and p.status=1  ",
				RDV.class);

		return list.setParameter("id", id).getResultList();
	}
	// SELECT * FROM `medicalpath`,pathdoctors p WHERE p.doctor_id=1

	@Override
	public List<MedicalPath> getAllIncludedMedicalPath(int id) {

		TypedQuery<MedicalPath> list = em
				.createQuery("select m from MedicalPath m,PathDoctors p WHERE p.doctor.id =:id ", MedicalPath.class);
		List<MedicalPath> l = list.setParameter("id", id).getResultList();
		for (MedicalPath m : l) {
			Set<PathDoctors> s = new HashSet<PathDoctors>();
			s.addAll(m.getDoctorPath());
			List<PathDoctors> pathdoc = new ArrayList<PathDoctors>();
			pathdoc.addAll(s);
			m.setDoctorPath(pathdoc);
		}
		return l;
	}

	@Override
	public MedicalVisit getVisitByPathId(int pathid, int iduser) {
		MedicalVisit v = new MedicalVisit();
		MedicalPath m = em.find(MedicalPath.class, pathid);
		for (PathDoctors p : m.getDoctorPath()) {
			if (p.getDoctor().getId() == iduser) {
	...
	@Override
	public void updateMedicalvisit(MedicalVisit medicalVisit){
		 MedicalVisit m= em.find(MedicalVisit.class, medicalVisit.getId());
	        m.setDescription(medicalVisit.getDescription());
	        m.setMedicalState(medicalVisit.getMedicalState());
	}
	
	@Override
	public List<User> getDoctors(int id){
		TypedQuery<User> list = em
				.createQuery("select u from User u where u.role=2 and Not exists (select p from PathDoctors p where p.path.id=:id and p.doctor.id=u.id)", User.class);
		//List<MedicalPath> l = list.setParameter("id", id).getResultList();
	...
	@Override
	public List<MedicalPath> getCompletedPath(int id) {
		// TODO Auto-generated method stub
		TypedQuery<MedicalPath> list = em
				.createQuery("select m from MedicalPath m where m.active=false and m.status=true and m.rendezVous.users.id=:id", MedicalPath.class);
		//List<MedicalPath> l = list.setParameter("id", id).getResultList();
	...
	@Override
	public List<MedicalPath> getunCompletedPath(int id) {
		// TODO Auto-generated method stub
		TypedQuery<MedicalPath> list = em
				.createQuery("select m from MedicalPath m where m.active=true and m.status=false and m.rendezVous.users.id=:id", MedicalPath.class);
		//List<MedicalPath> l = list.setParameter("id", id).getResultList();
